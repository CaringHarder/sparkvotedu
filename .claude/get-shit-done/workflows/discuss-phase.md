...
**CRITICAL: Interactive session protection.**
When this workflow is triggered by a slash command, AskUserQuestion may auto-complete with empty answers in the first model turn. To prevent this:
1. In your FIRST response, do steps initialize → check_existing → scout_codebase → analyze_phase
2. Present your gray area analysis as TEXT output (do NOT call AskUserQuestion yet)
3. End your first response with: "Reply with **ready** and I'll start the discussion."
4. WAIT for the user to respond before calling ANY AskUserQuestion
5. Only after receiving a user message, proceed to present_gray_areas and use AskUserQuestion
