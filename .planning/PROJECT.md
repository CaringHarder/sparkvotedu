...
- Device fingerprinting improvements -- fundamentally broken on identical school hardware; replaced by name-based identity in v1.2
...
**Classroom testing:** Name-based student identity (v1.2) working well -- replaces failed device fingerprinting. All bracket types and polls tested with real students.

**Known items for future work:**
- Microsoft and Apple OAuth configured in code but held (Google + email/password only)
- FingerprintJS package and device fingerprint columns still in codebase (cleanup deferred)
